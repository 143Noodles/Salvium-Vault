...
const STORAGE_FILE = process.env.STORAGE_FILE || '/tmp/vault-test-storage-export.json';
...
async function clickText(send, source, timeoutMs = 15000) {
  ...
  while (Date.now() < deadline) {
    const clicked = await evalValue(send, `(() => {
      ...
      for (const el of [...document.querySelectorAll('button,[role=button],a,label,div[tabindex]')].filter(visible)) {
...
async function importStorage(send, jsonText) {
  await evalValue(send, `window.__vaultImportChunks = []; true`);
  const chunkSize = 192 * 1024;
  for (let offset = 0; offset < jsonText.length; offset += chunkSize) {
    const chunk = jsonText.slice(offset, offset + chunkSize);
    await evalValue(send, `window.__vaultImportChunks.push(${JSON.stringify(chunk)}); window.__vaultImportChunks.length`);
    if (offset === 0 || offset + chunkSize >= jsonText.length || offset % (chunkSize * 20) === 0) {
      log('storage chunk', Math.min(jsonText.length, offset + chunkSize), '/', jsonText.length);
    }
  }
  return evalValue(send, `(async () => {
    const payload = JSON.parse(window.__vaultImportChunks.join(''));
    window.__vaultImportChunks = [];
    ...
    const decode = (value) => {
      ...
      if (value.__vaultType === 'ArrayBuffer' || value.__vaultType === 'TypedArray' || value.__vaultType === 'Blob') {
        const binary = atob(value.base64 || '');
        const bytes = new Uint8Array(binary.length);
        for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
        if (value.__vaultType === 'ArrayBuffer') return bytes.buffer;
        if (value.__vaultType === 'Blob') return new Blob([bytes], { type: value.mime || '' });
        if (value.ctor && typeof window[value.ctor] === 'function' && value.ctor !== 'DataView') {
          return new window[value.ctor](bytes.buffer);
        }
        return bytes;
      }
      if (value.__vaultType === 'Date') return new Date(value.value);
      if (Array.isArray(value)) return value.map(decode);
      const out = {};
      for (const [key, child] of Object.entries(value)) out[key] = decode(child);
      return out;
    };
    const existing = await indexedDB.databases();
    for (const db of existing) {
      if (db.name) {
        try { await req(indexedDB.deleteDatabase(db.name)); } catch {}
      }
    }
    localStorage.clear();
    ...
    const summary = [];
    for (const dbMeta of payload.databases || []) {
      const db = await new Promise((resolve, reject) => {
        const open = indexedDB.open(dbMeta.name, Math.max(1, Number(dbMeta.version || 1)));
        open.onupgradeneeded = () => {
          const db = open.result;
          for (const storeMeta of dbMeta.stores || []) {
            const options = {};
            if (storeMeta.keyPath !== null && typeof storeMeta.keyPath !== 'undefined') options.keyPath = storeMeta.keyPath;
            if (storeMeta.autoIncrement) options.autoIncrement = true;
            const store = db.createObjectStore(storeMeta.name, options);
      ...
      const tx = db.transaction((dbMeta.stores || []).map((store) => store.name), 'readwrite');
      for (const storeMeta of dbMeta.stores || []) {
        const store = tx.objectStore(storeMeta.name);
        for (const entry of storeMeta.entries || []) {
          const key = decode(entry.key);
          const value = decode(entry.value);
          if (storeMeta.keyPath === null || typeof storeMeta.keyPath === 'undefined') {
            store.put(value, key);
          } else {
            store.put(value);
          }
        }
        summary.push({ db: dbMeta.name, store: storeMeta.name, entries: (storeMeta.entries || []).length });
      }
      await txDone(tx);
      db.close();
    }
    return { localStorageKeys: Object.keys(payload.localStorage || {}).length, summary };
  })()`);
}

async function unlock(send, label) {
  const s = await state(send);
  log(label, JSON.stringify(s));
  if (s.inputs.some((input) => input.type === 'password')) {
    await fill(send, 'input[type=password]', PASSWORD);
    await clickText(send, 'unlock|continue|open', 8000);
  }
  return waitDashboard(send, label + '-sync', 120000);
}

(async () => {
  const jsonText = fs.readFileSync(STORAGE_FILE, 'utf8');
  startFreshApp();
  await sleep(7000);
  ...
  await sleep(3000);
  log('initial', JSON.stringify(await state(send)));
  const importResult = await importStorage(send, jsonText);
  log('imported', JSON.stringify(importResult));
  if (EXPECTED_ORIGIN) {
  ...
  const firstUnlockMs = await unlock(send, 'android-imported-unlock');
  const reloadStart = Date.now();
  await evalValue(send, `location.reload(); true`);
  await sleep(3000);
  const reloadUnlockMs = await unlock(send, 'android-imported-reload');
  ...
  console.log('ANDROID_IMPORTED_CACHE_RESULT ' + JSON.stringify({
    session: finalState.session,
    firstUnlockMs,
    reloadDashboardMs: reloadUnlockMs,
  ...
  console.error('ANDROID_IMPORTED_CACHE_FATAL', error && error.stack || error);
  process.exit(2);
});
